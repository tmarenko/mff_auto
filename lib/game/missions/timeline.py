...
class TimelineBattle(Missions):
    ...
    def battle_over_conditions(self):
        def points():
            return self.emulator.is_ui_element_on_screen(self.ui['TIMELINE_POINTS'])
    ...
    def open_timeline_battle(self):
        ...
        if wait_until(self.emulator.is_ui_element_on_screen, timeout=3,
                      ui_element=self.ui['TL_NEW_SEASON_NOTIFICATION']):
            logger.debug("Timeline Battle: found new season notification, closing.")
            self.emulator.click_button(self.ui['TL_NEW_SEASON_NOTIFICATION'].button)
        if wait_until(self.emulator.is_ui_element_on_screen, timeout=3, ui_element=self.ui['TL_LEAGUE_NOTIFICATION']):
            logger.debug("Timeline Battle: found league notification, closing.")
            self.emulator.click_button(self.ui['TL_LEAGUE_NOTIFICATION'].button)
        if not wait_until(self.emulator.is_ui_element_on_screen, timeout=3, ui_element=self.ui['TL_GET_READY_BUTTON']):
            logger.error("Timeline Battle: can't find GET READY button.")
            return False
        self.emulator.click_button(self.ui['TL_GET_READY_BUTTON'].button)
        if wait_until(self.emulator.is_ui_element_on_screen, timeout=3,
            ...
            self.emulator.click_button(self.ui['TL_RESTRICTED_NOTIFICATION'].button)
        if not wait_until(self.emulator.is_ui_element_on_screen, timeout=3, ui_element=self.ui['TL_SEARCH_BUTTON']):
            logger.error("Timeline Battle: can't find SEARCH button.")
            return False
        if wait_until(self.emulator.is_image_on_screen, timeout=1, ui_element=self.ui['TL_REPEAT_TOGGLE']):
            logger.debug("Found REPEAT toggle active. Clicking it.")
            self.emulator.click_button(self.ui['TL_REPEAT_TOGGLE'].button)
        self.select_team()
        self.emulator.click_button(self.ui['TL_SEARCH_BUTTON'].button)
        if wait_until(self.emulator.is_ui_element_on_screen, timeout=3, ui_element=self.ui['TL_HONOR_TOKENS_LIMIT']):
            ...
            self.emulator.click_button(self.ui['TL_HONOR_TOKENS_LIMIT'].button)
        if wait_until(self.emulator.is_ui_element_on_screen, timeout=3, ui_element=self.ui['INVENTORY_FULL']):
    ...
    def _search_for_new_opponent(self, skip_opponent_count):
        ...
        if skip_opponent_count <= 0:
            return
        if wait_until(self.emulator.is_ui_element_on_screen, timeout=3, ui_element=self.ui['TL_SEARCH_NEW_OPPONENT']):
            logger.debug(f"Skipping {skip_opponent_count} opponents.")
            for _ in range(skip_opponent_count):
                self.emulator.click_button(self.ui['TL_SEARCH_NEW_OPPONENT'].button)
                r_sleep(1)

    def _start_fight(self):
        """Go to fight screen and fight."""
        if not wait_until(self.emulator.is_ui_element_on_screen, timeout=3, ui_element=self.ui['TL_FIGHT_BUTTON']):
            logger.error("Can't find FIGHT button.")
        logger.debug("Starting the fight.")
        self.emulator.click_button(self.ui['TL_FIGHT_BUTTON'].button)
